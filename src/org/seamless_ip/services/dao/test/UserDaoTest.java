...
public class UserDaoTest {
	...
	public void allUsersTest() {
		List<UserTO> allUsers = users.findAll();
		assertEquals(2, allUsers.size());
		assertEquals("seamim", ((UserTO) allUsers.toArray()[0])
				.getAccountName());
		assertEquals("seampe", ((UserTO) allUsers.toArray()[1])
				.getAccountName());
	}

	@Test
	public void userAuthenticationTest() {
		UserTO user = users.findByAccountNameAndPassword("seamim", "seamim");
		assertNotNull(user);
		assertEquals("seamim", user.getAccountName());
	}
